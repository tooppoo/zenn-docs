...
>
> この原則はトム・デマルコとメイリル・ページ＝ジョーンズの著作で説明された。彼らはこれを凝集度と呼んだ。彼らは、モジュールの要素間の機能的関連性として凝集度を定義した。本章ではその意味を少し変え、凝集度をモジュールやクラスに変化をもたらす力と関連づける。
>（以降、ASD, Blogの翻訳は筆者による）
...
このように、SRPはパーナスに端を発し、ダイクストラ、コンスタンティン、デマルコ、ジョーンズらによって洗練されてきたモジュール分割の理論を統合したものである。
...
ASDにおいて、MartinはSRPに違反することの問題点を、「技術的効率の損失」と「設計の脆弱性」の2点に分けて挙げている。

#### 問題1：技術的効率の損失

Martinは例として、以下のような`Rectangle`クラスを挙げている。このクラスには、幾何学的計算（面積計算）と画面描画の二つのメソッドがある。これは、SRP違反の例である。[^7]
...
問題の本質は、「GUI技術の変更」という変更理由と、「幾何学的計算方式の変更」という変更理由が、一つのクラスに混在していることである。
...
Rectangleの例で言えば、GUIの変更が`Rectangle`クラスに変更を要求した場合、その変更が幾何学的計算に予期せぬ影響を与える可能性があるというのが、これにあたる。
...
「バグ防止」「変更容易性」「単一機能」は、この主目的の**帰結**としてしばしば現れるものであり、主目的そのものではない。
...
>
> これらの2つの責任は分離すべきでしょうか？それはアプリケーションがどのように変化するかによる。
> アプリケーションの変更が接続関数のシグネチャに影響を与える場合、設計は「硬直性」の臭いを放つことになる。なぜならsendやrecvを呼び出すクラスが、望ましくない頻度で再コンパイルと再デプロイを強いられるからだ。
> (...)
...
つまり、v1は**実際に発生した変更へ適用する**原則である。「あらかじめ適用しておく」という性質のものとしては、Martinにおいては捉えられていない。
...
この記事において、MartinはSRPの「変更の理由」を、**人々**に明示的に結びつけている。
...
**無関係なアクター（ここではCFO）まで影響を受ける可能性がある**。これが、v2で指摘されているもう一つの問題である。
...
v1とv2のいずれも、**クラスレベルでの分割**を扱う原則である。
...
永続化技術の変更は、**どの業務機能からも独立**して発生しうる。これを「アクターからの要求」として表現するのは不自然だ。

### 理由3. アクター同一性の循環

もう一つの問題がある。v2におけるアクター定義自体に**循環**がある（Martin 2017, p.82）。
...
このことから、SRPを実践において活用するならば、**v2のみならずv1の観点も併用する**ことが有効だと考える。

## 関心の種類によるv1・v2使い分け

前セクションの分析を踏まえ、本記事では以下を提案する。

**v1とv2は、同じクラスに対して、異なる種類の関心に基づいて使い分けられるべきである。**
...
懸念点： Blogでのアクター定義「業務機能を代表する人々のグループ」と矛盾する。永続化技術の変更は、すべてのアクター（CFO、COO、顧客など）が必要とする横断的関心であり、特定のアクターに帰属させるのは不自然だ。
...
懸念点： v1（ASD）では組織的関心の例がほとんどなく、技術的例が中心である。v2の「アクター」概念は、組織的関心を扱うための明示的な拡張と解釈すべきだろう。
...
懸念点： Martinが技術的分離の説明でアクターを使わず「関心の分離」に言及している点と矛盾する[Martin, 2014]。
...
しかし、SRPの「責任」を日常語の延長で理解するのではなく、Martinの原典に基づいて理解することの重要性は明らかだ。v1とv2の違いを認識し、適切に使い分けることで、より効果的なクラス設計が可能になるだろう。
