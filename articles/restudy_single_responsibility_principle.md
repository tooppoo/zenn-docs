...
v1では「変更の理由」とやや抽象的な定義だった「責任」が、「アクター」という言葉を用いることでより明確化されている。
...
結果、間違ったデータを出力してしまい、何百万ドルもの損害が出たというシナリオがここの説明で用いられている[^11]。
...
**Rectangle**（ASD p.96）：
- GUI描画 vs 幾何学的計算
- 変更理由：GUI技術の変更 vs 数学的アルゴリズムの変更

**Modem**（ASD p.97）：
- 接続管理 vs データ通信
- 変更理由：通信プロトコルの変更 vs データ処理の変更

これらはいずれも「変更の理由」（v1）で説明されており、その変更理由は**技術的変更**だ。そして、これらの例はCAではほぼ消えている。CAでは`Employee`という組織的な例が中心となり、技術的な例は周辺的になっている。

この変化は何を意味するのか？単に説明の都合なのか、それともv2が扱う問題の性質が変わったことを示しているのか？
...
この定義に照らして、DBAやCTOは「業務機能を代表する」のか？

- CFOは財務という業務機能を代表する ✓
- COOは人事・運用という業務機能を代表する ✓
- CTOは...技術インフラを担当する

CTOやDBAは**業務機能**ではなく、**技術的決定**を担当する。給与計算や勤怠管理といった「業務」ではなく、データベース技術の選定やスキーマ設計といった「技術」を扱う。

これはBlogでの定義—「業務機能を代表する人々のグループ」—と整合しない。

### 永続層とアプリケーション層の分離

この問題をより明確に示すため、一般的な設計原則を考えよう。

**設計原則：** 永続層（データアクセス）とアプリケーション層（ビジネスロジック）を分離せよ。
...
v2の「アクター」概念は、組織構造（部門、役職、ステークホルダー）を反映させる際には強力だ。異なる部門の要求が互いに干渉しないようにモジュールを分離するという目的には、明確で実践的な指針を与える。

しかし、永続層の分離、UI層の分離、技術スタックの変更といった技術的関心を「アクター」として表現するのは不自然だ。これらの変更理由は、特定の人々のグループではなく、技術的決定やアーキテクチャ上の制約から生じる。
