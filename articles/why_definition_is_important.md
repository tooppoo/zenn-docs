...
また、本稿は「定義を読むべき」という道徳を言いたいのではない。本稿が言うのは「定義を無視すると、議論と評価が壊れる」という話だ。そしてそれは、かえって「問題解決の速度」そのものを落としてしまう。
...
### 第二の問題：「定義」と「問題解決」は対立しない

この主張は、「定義の理解」と「問題解決」を対立関係として提示している。
しかし実際には、これらは対立ではなく**相補的関係**にある。

誤解された概念を用いて「解決したつもり」になることは、問題解決ではない。
概念の正確な理解がなければ、何が問題かすら正しく認識できない。

問題解決を重視するなら、むしろ、その前提として概念の定義を把握する必要がある。
「定義か、問題解決か」という構図そのものが、偽の二分法である。

### 第三の問題：自身が使う概念の定義は不問にする
...
泥沼化するのは、各自が異なる暗黙の定義を前提にしているからだ。
