...
ソフトウェア開発の文脈で、「専門用語の定義を追うのは無意味だ、エッセンスを利用すれば良い」という主張を見聞きした。
本稿では、この主張は日常語に対するものではなく、「設計原則や開発手法といった、共有知として流通している概念」に向けられたものとして扱う。
その上で、上記に対する所感をここで記しておく。

---

TDD、OOP、その他設計原則――そうした言葉の定義を詰めるより、目の前の問題解決に集中すべきだ、そのために必要なエッセンスのみを利用せよ、というのが件の主張だ。

一見すると実務的で、現場感覚に根ざした意見と思える。  
しかし私は、この主張は**問題解決を重視するという態度そのものと、実は両立していない**と考えている。

## 本稿の射程

本稿が批判的対象として扱うのは、以下の条件を**すべて**満たす言説である。

1. TDDやOOP、設計原則といった共有知として流通している専門概念が対象とする
2. その由来・原典・定義を確認する行為そのものを無意味あるいは有害とみなす
3. その結果生じる概念のズレや評価の歪みを問題視しない
4. 概念のズレや評価の歪みを、出典・原典との照合や留保の明示をせずに、外部の文脈へ持ち出す

1.を満たさないのであれば、それは日常の会話である。
本稿は、日常会話における言葉を制限・規定する意図を持たない。

2.を満たさないのであれば、それは本稿とは対立しない。
むしろ立場を同じくするものである。

3.を満たさない場合、さらに2.を満たすのであれば、ズレや評価の歪みを「問題」と呼ぶための参照点を否定したまま、
それを問題視することになり、論理的に自己矛盾を含む。

4.を満たさないのであれば、それはごく限定された文脈に閉じた話である。
1-3.が満たされている時点で問題はあるが、それが外部に持ち出されず閉じた文脈内部で完結しているならば、外部への影響は、一旦は抑制される。

## 冒頭の主張の問題点

冒頭で紹介した主張——「定義を追うのは無意味だ、エッセンスを利用すれば良い」——には、看過できない論理的問題が含まれている。

### 1. 「エッセンス」の特定は定義理解を前提とする

「定義を追うのは無意味」と述べながら「エッセンスを利用する」と語る。
しかし、**定義を理解せずに、どうやって「エッセンス」を特定するのか**。

エッセンス（Essence）とは本質、つまり概念の核心的な定義要素である。
ある概念の本質を把握するには、その概念の定義、すなわち、何が本質的で、何が付随的か、それを理解する必要がある。

地図を読むことを否定しながら「この道の本質を理解している」と主張するのと同じ構造だ。これは論理的に成立しない。

### 2. 「定義」と「問題解決」は対立しない

この主張は、「定義の理解」と「問題解決」を対立関係として提示している。
しかし実際には、これらは対立ではなく**相補的関係**にある。

定義を理解することによって、問題解決に対する理解がより鮮明になる。
また、問題解決という明確な目的を通じてさまざまな定義に触れることで、定義の理解が進む側面もある。

問題解決の達成を重視するなら、むしろ、その前提として概念の定義を把握する必要がある。
「定義か、問題解決か」という構図そのものが、偽の二分法である。

### 3. 「定義」の取り扱いに対する非対称性

例えば、「テスト容易性を高める目的でTDDのエッセンスを利用する」と述べた場合、そこには少なくとも以下の概念が含まれる。

- テスト容易性
- 目的
- TDDのエッセンス

これらはすべて抽象概念であり、それぞれ定義と理解が必要である。
「テスト容易性とは何か」「どの状態を指してテストが容易と言うのか」。これらの言明にも、定義の問題が含まれる。
それらを把握・確認する行為を“無意味”と言うと、言説の足場が崩れてしまう。

定義を軽視する立場は、自分が使う概念については暗黙の定義を前提としながら、他者の概念の定義だけを「無意味」と切り捨てる。この言説は、定義の扱いにこうした非対称性を導入してしまう問題がある。

---

以上の三点から、冒頭の主張は論理的に一貫していない。

ここで問題なのは誤解の有無ではなく、主張の形が自己崩壊しうる点だ。
自分が使う概念の定義は前提としながら、他者の定義参照を否定する。この言説構造には、論理的非対称性が含まれている。
...
TDDを例に取ろう。TDDは、Kent Beckみずからその誤解を解こうと試みなければならなかったほど[^1]に、解釈ズレの深刻な時期があった。
...
1. TDDの誤解から、Red-Green-Refactorのうち、Refactorが省略される
2. 技術的負債が蓄積
3. 3ヶ月後、「リファクタリング専用スプリント」を設けざるを得なくなる
4. 本来なら防げた1週間のコストが発生
5. 「TDDは冗長で重い」と評価される

といった事態に落ちうる危険性が十分にある。
<!-- textlint-enable ja-no-mixed-period -->
...
[^4]: Alan Kay(1998)<br/>> The big idea is "messaging" - that is what the kernel of Smalltalk/Squeak is all about (and it's something that was never quite completed in our Xerox PARC phase). The Japanese have a small word - ma - for "that which is in between" - perhaps the nearest English equivalent is "interstitial".
...
これらの記述から私が解釈する限り、これら2つのOOPは、少なくとも、その核心的な問題意識を異にしている。

- オブジェクト間の相互作用（messaging）を重視する、Smalltalk系
- オブジェクトの「内部構造（データ抽象化）」を重視する、C++系

この核心的な違いを無視したまま「OOP」という単一概念で語ると、どちらの文脈の話をしているのかが不明確になる。
...
こうした経緯を無視したまま、例えば「OOPとは変更容易性を高めるための手段だ」とだけ語ると、「どのOOPの話か」が消失する。あるいは、どちらのOOPについての注釈無く「OOPとは何か」とだけ語ろうとすると、これら両者の文脈が混在し、定義不能に陥る。
...
SRPを単なる道徳論として曖昧なままに運用すれば、以下のようなシナリオも十分に考えうる。

1. 「複雑そうだから分割しよう」という判断基準になる
2. 変更理由とは無関係な分割が行われる
3. クラス数の増加と認知負荷の増大
4. 認知負荷の増大→開発速度が低下
5. 「SRPは役に立たない」という誤った評価

ここでは2.がポイントとなる。「変更理由に基づいて分割せよ」という原典の基準を把握していれば、そもそも分割を回避できただろう。あるいは、「これは変更理由が一致していそうだが、～という理由で分離させておいた方が良い」という、より高精度な判断・評価ができただろう。
...
だが、本来の定義に従わない「TDD/OOP/SRPのような何か」が広まってしまい、それらを根拠としてTDD/OOP/SRPに対して下されたネガティブな評価は、原典で定義された概念に対する適切な評価と言えない。
...
実務においては、例えば次の程度の行為を想定すれば良いだろう。

- その言葉が指す意味内容の明示（「定義」の明示）
- 当面の作業範囲での暫定固定（このPR/この設計判断ではこれで行く）による合意
- 「合意」は運用上の収束条件で止める
    - 例：会話・ドキュメント・コードの三点で矛盾が出ないところまで

重要なのは、定義を暗黙のまま放置しないことだ。個々の実務という限定された文脈においてであれば、ここまでが実践されていれば十分だろう。
閉じた実務文脈においては、その意味内容さえ明確に定まっていれば、その参照点（原典など）は必ずしも必須としなくても間に合うことはある。

とはいえ、これで十分に正当化されるのは、上記のように閉じた文脈・グループ内部で限定的に用いられている限りにおいてである。
その「定義」が、参照点や留保を欠いたまま外部へ持ち出され、しかもそれが、既存の専門概念と**同一の名称（ラベル）**を用いて語られる時には、その限りではない。
本稿で問題視しているのは、この「外部へ持ち出す」ことにおいてである。

社内略語を業界標準語のような顔で話す行為を想像してほしい。このケースはさらに悪いことに、その略語が偶然にも業界標準語と同じ名前を持っている場合にも似ている。
この場合、意味のズレは極めて検知されにくくなり、しかも評価のすり替えが容易に起きてしまう。

概念を外部に持ち出すのであれば、最低限、原典・出典への参照の不十分さや不明瞭さは開示されねばならない。

- その言葉が、誰の・どの文脈の定義かを明示する
- 「自分はこの意味で使っている」と明示する
- 由来不明なら、由来不明であることを開示した上で用いる

なぜなら、こうした開示無しに外部へ持ち出すと、既に述べたような既存概念・公共知に対して混乱をもたらしうるからだ（TDD・SRP・OOPの節を参照）。
参照点が開示されていれば、読者側で出典を確認することで、その記述が妥当であるかどうかを検証できる。少なくとも、そのような検証を行える機会は提供できる。

限定的かつ閉じた文脈での運用方法と、その外部へ持ち出す場合とで、求められる基準は異なってくることに留意が必要だ。

## 想定される反論と再反論

### 「いちいち定義を追っていては時間がかかる、定義論争をしている暇はない」という態度について

このような態度を選択する人は、おそらく以下のような、あるいは近しい経験をしたことがあるのだろう。

- 「TDDとは何か」で会議が2時間空転した
- 「これはSRP違反か」で設計レビューが紛糾した
- 結局、定義の話は棚上げして実装を進めた

こうした経験から、定義論争を避けたいという態度は否定しない。むしろ正当な態度であると考える。
ただし、これらの原因が「定義を追求したからだ」と分析するのは、誤りであると判断する。
こうした問題の原因は、「定義を追求したこと」ではなく、「定義を参照しなかったこと」ではないだろうか。

「TDDとは何か」で揉めるのは、Kent Beckの記述に立ち戻ることをせず、「各々の考えるTDD」で論戦を行ったからではないだろうか。
「これはSRP違反か」で揉めるのは、Robert C. Martinの原典を誰も確認せず、「各々の考えるSRP（責任）」で考えたままに会話したからではないだろうか。

最初に出典を確認していれば、議論は30分で収束したのではないだろうか。
泥沼化するのは、各自が異なる暗黙の定義を前提にしているからではないか。

その意味で、 **定義論争を避けるためにこそ、定義を追いかけ、原典へ遡る必要がある**。
これは無矛盾であるどころか、むしろ、定義論争を最速で決着するための最も有効な方法であると考える。
...
「大体わかっている」という言明は、定義不要論ではなく、**定義の存在を前提とした省略表現**にすぎない。

問題は、その省略がどこまで許容されるかを判断する材料が、共有されていない点にある。

定義を明示的に追わない限り、「大体」の範囲は人ごとに異なり、再び解釈ズレが持ち込まれる。
つまり、「大体わかっていればいい」という態度は、「判断基準の導入は否定するが、"大体わかれば良い"という判断は肯定する」という自己矛盾を抱えている。
...
「TDDの定義は、正しくはこうだ」と主張しつつ、「このプロジェクトにおいて、TDDを用いるのは正しくない」と主張することは全くの無矛盾だ。
...
一方、実践における「正しさ」は「Aとは何か？」という定義の内容と、それが実際におかれた前提・文脈との兼ね合いで、ようやく実践レイヤにおける「正しさ」が決定する。つまり、実践のレイヤにおける「正しさ」とは、「Aとは何か？」と前提・文脈の複合体なのである。

「正しい」という言葉に惑わされてはならない。定義について語る時と、定義を用いる時とでレイヤが異なるのであり、それぞれで語られる「正しい」の意味内容は明確に異なっている。
...
**新しい意味を作ることと、既存の意味を曖昧に流用することは、全く別の行為だ**。前者は創造であり、後者は混乱である。
...
端的に言えば、「定義を追うのは重要でない」という考え方は、DDDの思想と正面から衝突する。
...
「定義は重要でない」という立場を取る限り、それは少なくとも、ユビキタス言語を中核に据えるDDDの立場とは相容れない。
これは、DDDが正しいから従え、という意味ではなく、「定義は重要でない」という立場が、ユビキタス言語という設計上の前提と両立しにくい、という意味である。
...
「定義を追う」「定義を確かめる」というのは、その定義を採用・肯定することを含めない。
定義を追った結果として肯定することもあれば、批判することも当然にある。

「定義を追う」とは、どの文脈の話をしているのかを特定し、解釈ズレを早期に検知し、批判と改善を可能にするための、**知的なデバッグ作業**である。

問題解決を本気でやるならばこそ、言葉から逃げることはできない。
...
- Bjarne Stroustrup(2005) 『C++の設計と進化』(ソフトバンク パブリッシング) 岩谷宏 訳
