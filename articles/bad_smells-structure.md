...
上記記事は、 ドメイン駆動設計に関するコミュニティであるDDD-Commumity-Jp（以下 「dddcj」）のDiscordサーバー内 #ModelingKai で現在(2020/12/19時点)も実施されている、モデリングを実際に実施・体験してみる会（以下 「モデリング会」）の体験から記述されています。
そこでは、モデリングの最中でも「コードの不吉な臭い」に類似した体験があること、「コードの不吉な臭い」同様にある程度の分類が可能であることが、それぞれ示されています。

本記事は、こうした「コードの不吉な臭い」がモデリングの最中でも、すなわちプログラムコード以外を対象とした活動においても見いだされた点に注目し、「不吉なにおい」というメタファを通じて私達が抱いている認識がどのような構造と性質を持っているのかについて、考察を試みたものです。
...
# 表記について
本記事では「臭い」 「匂い」 「におい」といった表記方法について、引用箇所を除いて、ひらがな表記の「におい」で統一します。引用箇所では、特に断りの無い限り、引用先の表記をそのまま利用します。引用先によって表記が異なる場合は、特に断りの無い限り、各引用先の表記をそれぞれ利用します。

# 「不吉なにおい」
ここでは、「不吉なにおい」という同一のメタファを用いながら、コードとモデル表現というそれぞれ異なる対象へとそのメタファを適用した２つの表現を比較します。
...
このように、「コードの不吉なにおい」は、あるコードを対象としたリファクタリングへ進む「きっかけ」を示します。「コードの不吉なにおい」を認識した人間は、その認識をきっかけにリファクタリングへ、すなわち「におい」を認識した対象であるコードへと「外部から見た時の振る舞いを保ちつつ、理解や修正が簡単になるように、ソフトウェアの内部構造を変化させ」る働きかけを試みるのです[^3]。
...
ここで見いだされた構造および性質はいずれも、対象がコードあるいはモデル表現であるが故に発生したものには思われません。むしろこの構造および性質こそ「不吉なにおい」という認識の基礎的部分であり、これがコードやモデル表現という具体例に適用した一例が、「コードの不吉なにおい」「モデルの不吉なにおい」として位置づけられるのではないでしょうか。
...
ここまでの議論と検討を通じて、私達はより一般的な概念としての「不吉なにおい」を手に入れたことになります。
...
この説明には、「重複したコード」という「不吉なにおい」を通じて確信された困難が大なり小なり減少するという確信が、すなわち「減少可能の確信」が含まれていると言って良いでしょう。

### 「義務の確信」
「重複したコード」が「不吉なにおい」として、すなわち「理解や修正が簡単になるように修正されるべきだ」という確信として認識されている時点で、そこには「修正されるべきだ」という確信が含まれています。
いわば、その種類を問わず、「不吉なにおい」として認識された対象は、ほぼ必然的に「義務の確信」その前提としていると考えて良いでしょう。
...
そうした直観は時に、論理的推論や情報の積み重ねからは到達できないような、飛躍を伴った課題解決方法、あるいは問題の発見へ至ることがあります。一方で、それは多くの場合に論理的推論や情報の分析を経由していないがために、他者への共有や、妥当性の検証が困難です。「不吉なにおい」が、単なる思い込みであった可能性も有るのです。
