...
5. **React18への移行**: 同様の指示 → **AIが生成したコードは100%そのまま動いた**
...
状態管理やビジネスロジックは、すべてドメイン層にある。View層は本当に「表示するだけ」だからこそ、AIであっても機械的に変換できたのだろう。

### ドメイン層の安定性

5年前のコードがそのまま動いている。Vue2・Vue3・React17・React18・jQueryのいずれにおいても、同じAPIを使える。フレームワークが変わっても、ビジネスロジックは変わらないまま、利用できている。
