...
以降では、いくつかの例を踏まえてその構造を説明していく。また、例について説明する際に参照した資料は、本稿末尾に参考資料として列挙している。
...
一見わかりやすい比喩ほど、本質を切除し、理解を固定化し、思考の幅を狭める危険がある。比喩は「類似点に着目して語る」という性質上、どうしてもこうした情報の切り捨てが発生するからだ。
しかし「わかりやすい」ことを上位の価値と位置づけてしまうと、こうした危険を無視ないし矮小化することを正当化する口実となってしまう。

比喩それ自体を全面否定したいわけではない。入門フェーズでの粗い足場としては有用な場面もある。
ただし本来は、その後に「ここから先は比喩を捨てよう」「この比喩では表現しきれない部分がある」という更新フェーズが必要だ。
その更新を怠ったまま比喩だけを量産すると、そこで理解が固定化される。

### 観点2. 説得力は、内容の正しさとは無関係に操作できる

説得力を指標にすると、『どれだけ正しいか』より『どれだけ筋が通っている**ように見えるか**』が先に評価される。

説得力とは、内容の正しさではなく **読者が受け取る印象の強さ** である。「説得力」とは「印象操作」と切り離せない側面も持っている。そのため、論理が粗かったり根拠が薄かったりしても、「説得力がある文章」は書けてしまう。
...
関数型言語は「副作用を禁止」しているというのは、少なくとも一般化としては正確ではない。
副作用を **分離** したり、**明示** する仕組みを持っていることは多いが、これらはいずれも「禁止」とは異なる。

例えばHaskellでは入出力という副作用を `IO` モナドの型の中に閉じ込め、純粋な計算と副作用を型レベルで分離している。入出力という副作用の利用そのものが「禁止」されているわけではない。[^5][^6]
...
説得力は、論理と独立した“印象操作”である以上、それに頼った説明を行ったり、それに頼った説明を鵜呑みにするのは危険である。
...
今度は、DDDの提唱者であるEric Evansの著書『エリック・エヴァンスのドメイン駆動設計』（以下、エヴァンス本）を参照してみる。
...
そしてここまで扱ってきたのは、「わかりやすさ」や「説得力」そのものではない。それらが**不透明性と結びついたときに何が起こるか**である。

比喩は情報を丸めて滑らかにする一方で、多くの情報を暗黙的に削り捨ててしまう。
説得力は、その一見した尤もらしさによって、論理の脆弱さや根拠の薄弱さを覆い隠してしまう。
引用先の欠落によって、誰かの解釈が権威を伴って広まり、しかしその原典へのアクセスは困難になってしまう。
...
- どの資料のどの箇所を読んでいるのかが示されない（出典の不透明性）

次節では、「不透明性」に対する対策としての「**透明性の倫理**」について述べる。
...
前節で挙げた三つの不透明性に対応するかたちで、「透明性の倫理」は次の三つの実践として整理できる。

- 限界の不透明性に対して：どこまで調べたか・どこからが推測かを示す「限界の開示」
- 範囲の不透明性に対して：事実と解釈と主張を分ける「範囲の開示」
...
技術記事の誤解が累積していく背景に、この“隠された不確実性”が紛れ込んでいることは珍しくない。
...
もちろん、あらゆる記事で一次資料を精査し、すべての引用にページ番号を振るのが現実的ではない場面も多い。商業書であればページ都合、WEBメディアであれば文字数上限といった制約もあるだろう。
...
なお、本稿で扱ったトピックについて、調査の射程も同一ではない。
たとえば、関数型言語と副作用の例については Haskell の入門書と haskell.jp の記事を主な対象としており、他の関数型言語や論文レベルの議論までは踏み込めていない。値オブジェクトについては Fowler と Evans の著作、およびそれらを参照する二次資料に依拠しており、DDD関連の他文献は今回の検討では対象外としている。クラスとインスタンスの比喩の例についても、言語仕様やリファレンスをざっと追うにとどまっており、オブジェクト指向研究全体を網羅しているわけではない。

SRPの節だけ透明度が高く見えるのは、実践例としてそこに追加コストをかけた結果であって、本稿全体が同じ水準で網羅的に調査されているわけではない。
...
ビジネスルールは頻繁に変更される傾向にあるが、永続化はそれほど頻繁に変更されなくとも、全く異なる理由で変更される。故に、この2つはほぼ混ぜるべきでないとしている。
...
ASDの説明は、複数の変更理由を抱えたモジュールが「再ビルド」「再テスト」「再デプロイ」などのコストを同時に引き起こすことを避ける、という意図が基盤にある。[^19]これは「凝集度」に近い古典的設計思想である。
...
この差異を無視して「SRPとは◯◯である」と一般化すると、SRPの説明は混乱を生じる。ASDに依拠した人とCAに依拠した人とで、会話が噛み合わないおそれもある。
...
少なくとも、「SRPとはXである」と断定したくなったときには、自分がASDの定義に立っているのか、CAの定義に立っているのか、そのどちらとも違う自分なりの再解釈なのかだけは、読者が判断できる形で示しておく必要がある。
...
SRPを題材にすると、その構造がよく見える。SRPに限らず、知識は本来こうした透明性の上で初めて理解可能になる。
