...
たとえば、関数型言語と副作用の例については Haskell の入門書と haskell.jp の記事を主な対象としており、他の関数型言語や論文レベルの議論までは踏み込めていない。値オブジェクトについては Fowler と Evans の著作、およびそれらを参照する二次資料に依拠しており、DDD 関連の他の文献は今回は検討対象外としている。クラスとインスタンスの比喩の例についても、言語仕様やリファレンスをざっと追うにとどまっており、オブジェクト指向研究全体を網羅しているわけではない。

SRP の節だけ透明度が高く見えるのは、単にそこに追加コストをかけた結果であって、本稿全体が同じ水準で網羅的に調査されているわけではない。
