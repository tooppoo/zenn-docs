...
さらに、作業空間を分けたとしても、複数のエージェントが同じファイルを編集してコンフリクトが発生する問題が残る。

前者は git worktree を使えばいいが、コマンドの使い方が若干面倒。
...
しかし最終的に変更を統合するとき、同じファイルに対する変更が衝突する可能性がある。

これは、従来のチーム開発でもよく起きていた問題である。複数の開発者が同じファイルを変更していれば、merge時にconflictが発生する。
そこでconflictの原因となったPRの作成者を探し、変更の意図を確認し、時には一緒にdiffを眺めながら最終的にどういう形になっているべきかを確認しあう、などしたこともあるかもしれない。

閑話休題。

AIエージェント時代には、この問題がローカル環境でも起きやすくなる。
人間が複数のAIエージェントをローカルで同時に動かすと、1人の開発者の手元で、複数人開発のような競合が発生する。

今まではチーム開発上の問題だったものが、ローカルな開発環境の問題として現れてくる。
...
それにあたり重要なのは、「conflictが起こらない分割を行う」ではなく、「conflictが起きそうになった瞬間に検知できる」ということではないかと思う。
...
AIオーケストレータではなく、AIエージェントを起動したり、タスクを自動分配したりすることを目的にはしていない。

`git-kura` が扱うのは、もう少し低いレイヤーだ。
...
実際に、あるIssueの作業を進めようとしたとき、別のIssue用worktreeがすでに必要なファイルをclaimしていたため、AIエージェントが作業を停止したことがある。
...
ここで重要なのは、conflictの可能性があることが検知され、それを受けてAIが作業を止め、人間に確認したことにある。
...
git worktreeは机を増やす。
git-kuraは、どの机がどの書類を触ってよいかを調停する。

それが、git-kuraで実現したいことです。
